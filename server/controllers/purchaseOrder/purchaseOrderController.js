...
export const getStatementsHandler = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Failed to fetch statement data: ${error.message}` });
  }
};
...
export const getSupplierSummaryHandler = async (req, res) => {
  try {
    ...
    console.log(
      "Supplier summary result:",
      summary.length,
      "supplier-month combinations found"
    );
    res.json(summary);
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: `Failed to fetch supplier statements: ${error.message}` });
  }
};
...
export const getPurchaseOrderListHandler = async (req, res) => {
  try {
    ...
    const purchaseOrders = await getPurchaseOrderList(
      supplierId,
      expenseTypeId,
      fromDate,
      toDate,
