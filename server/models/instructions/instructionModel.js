...
export const updateInstruction = async (instructionId, updatedData) => {
  ...
  try {
    ...
    const getCurrentStatusQuery = `
      SELECT status FROM public.m1_controller WHERE m1key = $1
    `
    const currentStatusResult = await client.query(getCurrentStatusQuery, [instructionId])
    const currentStatus = currentStatusResult.rows.length > 0 ? currentStatusResult.rows[0].status : "New"

    console.log(`Preserving current status: ${currentStatus} for instruction ID: ${instructionId}`)
    ...
    const values = [
      ...
      updatedData.total_cost,
      updatedData.weight,
      currentStatus, // Use the preserved current status instead of updatedData.status
      bookingRef,
      vesselName,
      voyageNum,
      imoNum,
    ...
    console.log(`Successfully updated instruction ${instructionId} while preserving status: ${currentStatus}`)
