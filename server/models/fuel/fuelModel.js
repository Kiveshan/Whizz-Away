...
const getAllCompanyOwnedTrucks = async (company_reg_num) => {
  const query = `
    SELECT
...
const getAllExpenses = async (company_reg_num) => {
  const queryText = `
    ...
    WHERE e.company_reg_num = $1
    ORDER BY e.slipuploaddate DESC
  `;
  const result = await pool.query(queryText, [company_reg_num]);
  return result.rows;
};
