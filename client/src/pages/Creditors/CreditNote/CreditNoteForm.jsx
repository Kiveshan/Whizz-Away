  ...
  try {
    ...
    const fetchLatestDocumentNumber = async () => {
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          documentNo: "", // Fallback if API fails
        }));
      }
    };
...
useEffect(() => {
  const fetchInstructionDetails = async () => {
    console.log("Fetching instruction details for instructionNo:", formData.instructionNo); // Debug log
    if (formData.instructionNo) {
      try {
        const response = await api.get(`/api/instruction-details/${formData.instructionNo}`);
        const { dropoff, vessel_name, fileref } = response.data.data || {};
        console.log("Fetched instruction details:", { dropoff, vessel_name, fileref }); // Debug log
        setFormData((prev) => ({
          ...prev,
          destination: dropoff || "",
          vessel: vessel_name || "",
      ...
      try {
        ...
        console.log("Fetched containers:", response.data.data); // Debug log
      ...
      console.log("Clearing instruction details due to no instructionNo"); // Debug log
      setContainers([]);
      setFormData((prev) => ({
        ...prev,
        destination: "",
...
const creditNoteData = {
  client_id: clientId,
  creditnote_date: formData.documentDate,
  amount: amounts,
  containerids: containerIds, // No need for length check since filter handles empty cases
  ...
  return (
    <div className="credit-note-form-wrapper">
      <div className="credit-note-form-container">
        ...
        <h2 className="credit-note-form-title" style={{textAlign:"center"}}>CREDIT NOTE</h2>
