...
const FCcontrollerInstructionDetails = () => {
  ...
  console.log(
    "FCcontrollerInstructionDetails - preservedContainers:",
    preservedContainers
  );
  ...
  const handleBackClick = () => {
    ...
    console.log(
      "Navigating back to FCcontrollerinstructions with state:",
      stateToPass
    );
  ...
  useEffect(() => {
    ...
    if (preservedContainers && preservedContainers.length > 0) {
      console.log(
        "Using preserved containers from navigation:",
        preservedContainers
      );
  ...
  const syncContainersWithCounts = (containersList) => {
    ...
    if (
      ...
      result = [
        ...containersByType["6m"],
        ...containersByType["12m"],
        ...containersByType["Abnormal"],
      ];
    }
  ...
  const handleAddContainer = (containerType) => {
    ...
    setUpdatedControllerData((prevData) => {
      ...
      if (newData.rateWeight === "Container") {
        ...
        if (!isNaN(rate)) {
          const totalContainers =
            (newData.num_six_meters || 0) +
            (newData.num_twelve_meters || 0) +
            (newData.num_abnormal || 0);
    ...
    if (containerToDelete) {
      setUpdatedControllerData((prevData) => {
        ...
        if (containerToDelete.containerType === "6m") {
          newData.num_six_meters = Math.max(
            0,
            (newData.num_six_meters || 0) - 1
          );
        } else if (containerToDelete.containerType === "12m") {
          newData.num_twelve_meters = Math.max(
            0,
            (newData.num_twelve_meters || 0) - 1
          );
  ...
  const saveChangesToDatabase = async () => {
    try {
      ...
      if (finalControllerData.rateWeight === "Container") {
        ...
        if (
          !finalControllerData.weight ||
          isNaN(Number.parseFloat(finalControllerData.weight))
        ) {
          throw new Error(
            `Please enter a valid weight for ${finalControllerData.rateWeight} rate`
          );
        }
        finalControllerData.total_cost = calculateTotalCost();
      }
      ...
      const instructionData = {
        ...
        pickuptime: finalControllerData.pickupTime
          ? formatTimeForSubmission(finalControllerData.pickupTime)
          : null,
        pickupdate: finalControllerData.pickupDate
          ? formatDateForSubmission(finalControllerData.pickupDate)
          : null,
        stackdate: finalControllerData.stackDate
          ? formatDateForSubmission(finalControllerData.stackDate)
          : null,
        deadline: finalControllerData.deadline
          ? formatDateForSubmission(finalControllerData.deadline)
          : null,
        ...
        num_six_meters:
          Number.parseInt(finalControllerData.num_six_meters) || 0,
        num_twelve_meters:
          Number.parseInt(finalControllerData.num_twelve_meters) || 0,
        num_abnormal: Number.parseInt(finalControllerData.num_abnormal) || 0,
        weight:
          finalControllerData.rateWeight === "Container"
            ? null
            : Number.parseFloat(finalControllerData.weight) || 0,
      ...
      // Update instruction with better error handling using axios
      try {
        ...
        let errorMessage = "Failed to update instruction";
        if (error.response) {
          errorMessage = `Failed to update instruction: ${error.response.status} ${error.response.statusText}`;
      ...
      console.log(
        "Sending container data to API:",
        JSON.stringify(containerData, null, 2)
      );

      // Send container data to API with better error handling using axios
      try {
        const containerResponse = await api.post(
          `/api/containers/${instructionId}`,
          containerData
        );
        ...
        let errorMessage = "Failed to save containers";
        if (error.response) {
          errorMessage = `Failed to save containers: ${error.response.status} ${error.response.statusText}`;
        } else if (error.request) {
          errorMessage =
            "No response received from server. Please check your connection.";
        }
        ...
        <div className="content">
          ...
          >
            <p style={{ margin: 0, color: "#856404" }}>
              Note: Click "Save Changes" to save all your modifications.
            </p>
          </div>
